...
class StepFunctionsStateMachine:
    ...
    def start_run_sync(self, run_name, run_input=None):
        """
        Starts a run with the current state definition.
        ...
        try:
            ...
            if run_input is not None:
                kwargs['input'] = json.dumps(run_input)
            response = self.stepfunctions_client.start_sync_execution(**kwargs)
            #run_arn = response['executionArn']
            logger.info("Started sync run %s.", run_name)
        except ClientError:
            logger.exception("Couldn't start run %s.", run_name)
            raise
        else:
    ...
    def get_run_logs(self, execution_arn):
        """
        ...
        if self.state_machine_arn is None:
            raise ValueError
        try:
            kwargs = {'executionArn': execution_arn, 'maxResults':999}
            response = self.stepfunctions_client.get_execution_history(**kwargs)
            return response
        except ClientError:
            logger.exception("Couldn't describe run %s.", execution_arn)
            raise
        else:
            return {} 
    ...
    def describe_execution(self, execution_arn):
        ...
        try:
            ...
            response = self.stepfunctions_client.describe_execution(**kwargs)
            return response
        except ClientError:
            logger.exception("Couldn't describe run %s.", execution_arn)
            raise
